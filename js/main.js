...
// Função para calcular a multiplicação de binarios
function multBinarios (binario1, binario2){ // Passandro duas strings binarias como parâmetro
    // Declarando duas variaveis que irão receber o valor dos parametros da função convertidos pra base 2
    let int1 = parseInt(binario1, 2)    
    let int2 = parseInt(binario2, 2)
    return (int1 * int2).toString(2) // Retornando a multiplicação das duas variaveis criadas acima e convertendo-as novamente para binario
}

// Função para retornar no HTML o resultado final
